  ...
  return (
    <PageContainer as={"section"} id="Hero" py={SECTION_VERTICAL_PADDING}>
      <Grid templateColumns={["1fr", "1fr", "1fr 1fr"]} gap={20}>
        <GridItem>
          <Stack mt={[0, 0, 10]}>
            <Box textAlign={["center", "center", "left"]}>
              {HERO_TITLE?.split("\n").map((line, index) => (
                <TitleText fontWeight={"medium"} color={PRIMARY_COLOR} lineHeight={"1.3"} key={`hero-title-${index}`}>{ line }</TitleText>
              ))}
            </Box>

            <Text textAlign={["center", "center", "left"]} fontWeight={500} mt={4} maxW={["full", "full", "md"]} fontSize={"lg"}>{HERO_TEXT}</Text>
        ...
        <GridItem>
          <Image maxW={500} w={"100%"} mx={["auto", "auto", 0]} src={HERO_IMAGE} alt="Hero Image" />
        </GridItem>
      </Grid>
    </PageContainer>
  );
