      ...
      <SimpleGrid columns={[1, 1, 2]} gap={8}>
        <Stack spacing={10}>
          ...
          <HStack mt={10} spacing={4} justifyContent={["center", "center", "flex-start"]}>
            <Link href={links.ios} target="_blank" rel="noopener noreferrer" as={Image}  maxW={40} w={"100%"} src={icon.ios} alt="Download on App Store" />
            <Link href={links.android} target="_blank" rel="noopener noreferrer" as={Image}  maxW={40} w={"100%"} src={icon.android} alt="Download on Play Store" />
          </HStack>
        </Stack>

        <Box>
          <div data-aos="fade-left">
            <Image src={image} w={"100%"} maxW={500} alt={type + " app"} />
          </div>
        </Box>
      </SimpleGrid>
    </PageContainer>
